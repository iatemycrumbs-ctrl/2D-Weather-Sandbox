    ...
    if (water[CLOUD] > threshold && base[TEMPERATURE] < 2500.0) {
      ...
      if (spawnChance > nrmRand) { // spawn precipitation particle
        ...
        if (realTemp < CtoK(0.0)) {
          ...
          lightningSpawnChance *= (0.42 + electricPotential * 2.72);
          ...
          float stratiformFloor = max(cloudPlusPrecipDensity - (lightningCloudDensityThreshold + 0.03), 0.0) * 0.042;
          ...
          lightningSpawnChance = clamp(lightningSpawnChance, 0.00012, 0.92);
          ...
          bool overdueStormRecharge = lightningWarmupDone && lightningChannelFree && previousLightningAge > (48.0 + 20.0 * lightningMinInterval)
                                    && cloudPlusPrecipDensity > lightningCloudDensityThreshold + 0.17
