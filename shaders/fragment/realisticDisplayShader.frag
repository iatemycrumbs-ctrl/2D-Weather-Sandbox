...
{
  vec2 uv = baseCoord;
  uv.x *= scaleMult * aspectRatios[0] / lightningTexAspect * 0.92;
  uv.y *= -scaleMult * 0.72;
  uv.x += 0.5;
...
vec3 displayLightning(vec2 pos, float lightningTime, float currentLightningIntensity)
{
  ...
  if (strikeTypeSign < 0.0) {
    ...
    float cgSourceFade = smoothstep(pos.y + 0.04, pos.y - 0.01, texCoord.y);
    pixVal *= clamp(cgAboveCloudFade * cgSourceFade, 0.0, 1.0);
  }
