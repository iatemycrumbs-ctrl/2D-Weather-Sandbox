    ...
    if (wall[VERT_DISTANCE] < 0) {                                         // below surface
      ...
      case WALLTYPE_LAND:                                                                                          // no break,can also be fire or urban:
        ...
        if (int(iterNum) % 100 == 0) { // snow and soil moisture smoothing
          ...
          // lightning ignites at surface
          float lightningAge = interNum - lightningData[START_ITERNUM];
          if (wall[VERT_DISTANCE] == 0 && wall[TYPE] == WALLTYPE_LAND && wall[VEGETATION] >= minimalFireVegetation &&
              ...
              float dxCells = abs(textCoord.x - lightningData.x) * resolution.x;
              dxCells = min(dxCells, resolution.x - dxCells);
              ...
              float wetnessPenalty = clamp((waterAboveSurface[PRECIPITATION] * 0.12, 0.0, 0.95);
