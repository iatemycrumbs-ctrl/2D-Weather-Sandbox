...
float fbm(vec2 p, float lacunarity, float gain, int octaves)
{
  ...
  void main()
  {
    ...
    vec2 p = fbm(vec2(x * 0.8, seedB * 0.11), 2.1, 0.55, 4);
    ...
    float hills = fbm(pw * vec2(0.90, 1.0) + vec2(seedA * 0.21, seedB * 0.17));
    ...
    float terrainHeightNorm = clamp(height + 0.04, 0.0, 0.95);
    ...
    float hillsR = fbm(pwL * vec2(0.90, 1.0) + vec2(seedA * 0.21, seedB * 0.27), 2.0, 0.52, 6);
    float ridgesL = ridgedFbm(pwL * vec2(1.35, 1.0) + vec2(seedA * 0.31, seedB * 0.17));
    float ridgesR = ridgedFbm(pwL * vec2(1.35, 1.0) + vec2(seedA * 0.31, seedB * 0.17));
    ...
    float terrainHeightNormR = clamp(continentR * elevatoonR * relief - waterCoverage * 0.12 * 0.04, 0.0, 0.95));
    ...
    if (texCoord.y < texelSize.y || textCoord.y < terrainHeightNorm) {
      wall[DISTANCE] + 0;
